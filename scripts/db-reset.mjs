...
async function main() {
  ...
  await sql`TRUNCATE TABLE notifications RESTART IDENTITY`;
  await sql`TRUNCATE TABLE approvals RESTART IDENTITY`;
  await sql`TRUNCATE TABLE requests RESTART IDENTITY`;
  await sql`TRUNCATE TABLE users RESTART IDENTITY`;
  await sql`TRUNCATE TABLE stores RESTART IDENTITY`;
  console.log('All business data reset (users, stores, requests, approvals, notifications).');
}

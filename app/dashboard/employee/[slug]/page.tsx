...
const COLORS = ['#10b981', '#0ea5e9', '#f59e0b', '#ef4444', '#7c3aed', '#0284c7'];
...
export default function EmployeeDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const usersData = await readJsonSafely(usersRes) as FieldUser[] | null;
      const slugName = unslugify(slug).toLowerCase();
      const found = Array.isArray(usersData) ? usersData.find((u) => u.name.toLowerCase() === slugName) : undefined;
      if (!found) { setNotFound(true); setLoading(false); return; }
      setUser(found);
      const reqRes = await fetch(`/api/requests?targetUserId=${found.id}`, { cache: 'no-store' });
  ...
  const avgMonthly = trend.length > 0 ? Math.round(trend.reduce((s, t) => s + t.total, 0) / trend.length) : 0;

  const filteredRequests = useMemo(() =>
    periodRequests.filter((r) => !statusFilter || r.status === statusFilter),
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="container py-10">
        ...
        <div className="mb-6 flex items-center gap-2 text-sm text-slate-500">
          ...
          <span>›</span><span className="text-slate-500">Field Team</span>
          <span>›</span><span className="font-medium text-slate-800">{user.name}</span>
        </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-emerald-700">Field Team</span>
              {user.title && <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">{user.title}</span>}
            </div>
            ...
            <p className="mt-1 text-sm text-slate-500 flex flex-wrap items-center gap-2">
              Monthly budget: {formatCurrency(monthlyBudget)} · 6-month avg: {formatCurrency(avgMonthly)}
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-2">
          <Card className="gap-4">
            ...
            <div className="flex-1 min-h-[14rem]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trend} margin={{ top: 4, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fill: '#475569', fontSize: 12 }} tickFormatter={(v) => `€${(v / 1000).toFixed(1)}k`} />
                  <Tooltip formatter={(value: number) => formatCurrency(value)} />
                  <Line type="monotone" dataKey="total" stroke="#10b981" strokeWidth={2.5}
                    dot={{ fill: '#10b981', r: 4 }} activeDot={{ r: 6 }} name="Spend" />
                  <Legend />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div className="flex items-center gap-2 rounded-2xl bg-slate-50 px-4 py-2 text-sm">
              ...
              <span className="text-slate-500">6-month average: {formatCurrency(avgMonthly)} / month</span>
            </div>
          </Card>
                  ...
                  {byCategory.map((cat, i) => (
                    <div key={cat.category} className="flex items-center justify-between text-sm">
                      ...
                      <div className="text-right">
                        ...
                        <span className="ml-2 text-xs text-slate-400">{totalSpent > 0 ? Math.round((cat.total / totalSpent) * 100) : 0}%</span>
                      </div>
                    </div>
                  ))}
                  {topCategory && (
                  ...
                  <tbody>
                    {filteredRequests.map((req) => (
                      <TableRow key={req.id}>
                        ...
                        <TableCell><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">{req.category}</span></TableCell>
                        <TableCell className="max-w-sm text-sm text-slate-600">{req.description}</TableCell>
...
import { EMPLOYEE_DETAILS, type Period } from '@/lib/mockData';
...
export default function EmployeeDetailPage() {
  ...
  const router   = useRouter();
  const slug     = Array.isArray(params.slug) ? params.slug[0] : (params.slug ?? '');
  const employee = EMPLOYEE_DETAILS[slug];
  ...
  // Compute 6-month rolling avg for insight
  const avgMonthly = employee.trend.length > 0
    ? Math.round(employee.trend.reduce((s, t) => s + t.total, 0) / employee.trend.length)
    : 0;
      ...
      <main className="container py-10">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-emerald-700">Field Team</span>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">{employee.role}</span>
            </div>
            <h1 className="mt-2 text-3xl font-semibold text-slate-900">{employee.name}</h1>
            <p className="mt-1 text-sm text-slate-500 flex flex-wrap items-center gap-2">
              Monthly budget: {formatCurrency(employee.monthlyBudget)} · 6-month avg: {formatCurrency(avgMonthly)}
              {dateRange?.from && dateRange?.to && (
                <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700">
                  {format(dateRange.from, 'd MMM')} – {format(dateRange.to, 'd MMM yyyy')}
                </span>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-2">
          ...
          <Card className="gap-4">
            <div>
              <p className="text-sm uppercase tracking-widest text-slate-500">6-month spend trend</p>
              <p className="mt-1 text-xs text-slate-400">Monthly OPEX versus {formatCurrency(employee.monthlyBudget)} budget</p>
            </div>
            <div className="flex-1 min-h-[14rem]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={employee.trend} margin={{ top: 4, right: 10, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} />
                  <XAxis dataKey="month" tick={{ fill: '#475569', fontSize: 12 }} />
                  <YAxis tick={{ fill: '#475569', fontSize: 12 }} tickFormatter={(v) => `€${(v / 1000).toFixed(1)}k`} />
                  <Tooltip formatter={(value: number) => formatCurrency(value)} />
          ...
          <Card className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">
                Breakdown for {period === 'month' ? 'this month' : period === 'last-month' ? 'last month' : 'this quarter'}
              </p>
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                  ...
                  <tbody>
                    {filteredRequests.map((req) => (
                      <TableRow key={req.id}>
                        ...
                        <TableCell>
                          <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">{req.category}</span>
                        </TableCell>
                        <TableCell className="max-w-sm text-sm text-slate-600">{req.description}</TableCell>
                        <TableCell className="font-semibold text-slate-900 whitespace-nowrap">{formatCurrency(req.amount)}</TableCell>
                        <TableCell>
